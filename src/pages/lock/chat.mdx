...
*For an end-to-end security walkthrough of the whole app — identity exposure, device keys, relays, share URLs, recommended postures — see [`/lock/security`](/lock/security).*
...
| Stolen locked device | Local ciphertext + plaintext history cache. | *Still* needs the passphrase to unlock device_sk and continue receiving. Prior decrypted plaintext is recoverable iff the attacker breaks your passphrase. |
...
Chat is not a distinct product inside Lock — it's the auto-delivery transport every OC Lock envelope can ride. When you send a message in the unified `/app` inbox, the same envelope is published as a Nostr gift-wrap (for automatic delivery) AND made available as a share URL / `.lock` file / QR on the resulting bubble (for out-of-band hand-off). Recipients with a Lock device get it automatically; anyone else can still decrypt the URL. Closing the tab stops the subscription; opening `/app` again with an unlocked device resumes it.
