import { ConsoleWebhookEventCatalog } from '@/components/docs/ConsoleWebhookEventCatalog';
...
Subscribe an HTTPS endpoint at `/settings § 04 · webhook endpoints` and console
...
[`https://console.ochk.io/api/webhook-events`](https://console.ochk.io/api/webhook-events)
— the typed source of truth lives at `src/lib/webhooks/events.ts` in
[orangecheck/oc-console-web](https://github.com/orangecheck/oc-console-web), and
a CI contract test asserts every `dispatchEvent()` call uses an id from this
catalog.

<ConsoleWebhookEventCatalog />
...
When you create an endpoint, console returns the signing secret **once** in the
