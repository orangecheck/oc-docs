export const metadata = {
    ...
    description:
        '@orangecheck/me-client SDK reference. Three entry points: main (oc.* calls + OcSignInButton), /server (framework adapters — Next.js, Express, Hono), /popup (browser-only signin orchestrator). signInWithOc, oc.session, oc.payment, oc.event, oc.webhook, oc.config, oc.identity (cross-integrator sybil-resistance verifier) — plus every type the SDK exports.',
};

# `@orangecheck/me-client` · SDK reference

> **Full reference:** [docs.ochk.io/sdk/me-client](/sdk/me-client/README) —
...
**MIT.** Version-pinned reference: see
[docs.ochk.io/sdk/me-client](/sdk/me-client/README), auto-generated per release.

This is the canonical TypeScript SDK for integrating me.ochk.io. Three entry
points, three audiences:

- **`@orangecheck/me-client`** — React + JS surface: the `OcSignInButton`
  component plus the `oc.*` namespaces (session, payment, event, webhook,
  config, identity). The `<OcSessionProvider>` + `useOcSession()` React bindings
  ship in the companion package `@orangecheck/auth-client` — install both.
...
const app = express();
app.use(ocAuthExpress({ audience: 'https://your-site.example' }));
...
const app = new Hono();
app.use('*', ocAuthHono({ audience: 'https://your-site.example' }));
...
**`audience` is your own origin.** The sign-in popup issues your site a token
bound to it, and with `audience` set, a token issued to any other site is
refused. Leave it out only on a `*.ochk.io` site that reads the family cookie —
with an audience set, a plain family session is refused. Requires
`@orangecheck/me-client` 0.27 or later.
...
button.addEventListener('click', async () => {
    ...
    // Outside *.ochk.io, send the token to your backend as a Bearer; it
    // verifies it with the server adapters above.
    const me = await fetch('/api/auth/me', {
        headers: { Authorization: `Bearer ${result.token}` },
    });
    if (me.ok) location.assign('/dashboard');
});
```
...
> `@orangecheck/agent-signer` to issue or revoke agent delegations from your
> integration, and `@orangecheck/agent-core` to verify them.
...
back-dated, active_days requires habitual presence over time) and **trivial to
verify** (one function call, one signature check, identical posture on Node 20+,
Vercel Edge, browser).

The verifier displaces the adjacent category of spend integrators currently pay
for separately — fraud detection, anti-spam, account-takeover monitoring — with
a single primitive that ships as a side effect of using OC for auth.

> Same five checks run in-browser at
> [me.ochk.io/verify-activity](https://me.ochk.io/verify-activity) for any third
> party who receives a forwarded attestation but doesn't want to write the
> verifier themselves. The SDK and the browser page produce identical verdicts
> on the same input.
...
Call it from your backend. `signingSecret` signs the request as your project
(the SDK signs the exact bytes it sends), and `userAddress` names who earns —
the user's `did_oc`, stored beside your own user row. No user session token is
forwarded.

```ts
// server-side only · the secret never reaches a browser
const stamp = await oc.event.fire({
    project_key: 'pk_live_yourcompany',
    subtype: 'stamp_signing',
    action_label: 'review · march invoice',
    signingSecret: process.env.OC_SIGNING_SECRET,
    userAddress: user.did_oc,
});
...
`@orangecheck/me-client` does not re-export the React surface: import
`OcAccount`, `OcSessionState`, `OcSessionStatus` and `OcAuthConfig` from
