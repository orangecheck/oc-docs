...
The authenticated user is the cashback recipient; `project_key` (in the body)
identifies the billing project. Cookie **or** `Authorization: Bearer` — see the
note above. Sign the request with the project signing secret (`X-OC-Signature`)
— in **both** test and live mode. A user session proves a user, never the paying
project, so an unsigned POST is admitted only when the session belongs to the
project's own owner (the CLI test-fire path). Anything else gets
