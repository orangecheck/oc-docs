export const metadata = {
    title: 'walkthrough',
    description: 'Narrative companion to the spec. Five flows, concrete scenarios, realistic addresses and block heights.',
};

# walkthrough

Narrative companion to the [spec](/vote/spec). Five flows, concrete scenarios, realistic addresses and block heights.
...
```
  ...
  │          │    publishes kind 30080        │  directory   │
  └──────────┘                                │              │
                                              │ 30080 poll   │
  ┌──────────┐                                │ 30081 ballot │
  │ Voter A  │──── signs ballot (BIP-322) ──→│ 30082 reveal │
...
If Alice refuses to reveal, the poll is permanently **abandoned**. Participation is still provable; the tally is not. This is an honest trade-off — see [secret ballot](/vote/secret-ballot).
...
- **OrangeCheck** — voters may reference an existing OC attestation in `ballot.attestation_id` as a client hint. Authoritative check is always the UTXO snapshot.
...
- **OC Vote** — produces a signal. That signal can feed back into either primitive.

Three primitives, one substrate, zero tokens.
