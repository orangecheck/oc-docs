export const metadata = {
    ...
    description: 'Declarative scope strings, constraint operators, sub-scope containment, and the MVP constraint-key registry.',
};
...
New products, verbs, and constraint keys are allocated by PR to the [oc-agent-protocol](https://github.com/orangecheck/oc-agent-protocol) repository. Vendor-specific experimental values SHOULD use an `x-vendor/` prefix until standardized.

## Implementing the sub-scope check

The reference TypeScript implementation lives in [`@orangecheck/agent-core`](https://github.com/orangecheck/oc-packages/tree/main/agent-core):
