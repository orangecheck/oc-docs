...
export const meta = {
    ...
    description:
        'Summary of the normative v1 spec. The authoritative copy lives in the spec repo.',
};
...
<SpecRef
    ...
    kind="spec"
    note="This page is a summary keyed to the same section numbers, so you can navigate between the two. The authoritative copy lives in the spec repo."
/>

## Shape

- **§0 Notation** — `H()`, `BIP322()`, canonical JSON, ISO 8601 UTC.
- **§1 Actors** — signer, verifier, aggregator (optional), calendar, directory
  (optional).
- **§2 Identities** — signer = Bitcoin address (P2WPKH, P2TR, P2PKH). Optional
  `stake.attestation_id` points at an OrangeCheck attestation.
- **§3 Canonical message** — six lines, LF-separated, no trailing LF,
  `oc-stamp:v1` as domain separator.
...
A minimal verifier skips steps 5, 6, 7 and reports "authentic but not anchored /
content not checked." That's the right mode for preview UIs that only care about
authorship.

Full verification runs all seven and requires a block-headers source (full node,
SPV client, or a pre-computed snapshot). **No ochk.io endpoint is required.**
...
- Passes every committed test vector in
  [`test-vectors/`](https://github.com/orangecheck/oc-stamp-protocol/tree/main/test-vectors).
