...
reference is mirrored at [me.ochk.io/sdk](https://me.ochk.io/sdk) — this
page is the docs.ochk.io copy organized alongside the other product
references.
...
`@orangecheck/auth-client` is the React provider + hook. `me-client`
re-exports it, so most code only needs to import from `me-client`.
...
Returns `{ status, account, signInUrl, signOut, refresh, error }`.
Status is `loading | authenticated | anonymous | error`. Account is
...
`useOptionalOcSession()` returns null instead of throwing — useful for
libraries that want to read the session if it exists but shouldn't crash
on apps that haven't mounted the provider.
...
re-serialize before your handler produce a different byte sequence and
...
Auto-fetches and caches `ochk.io/.well-known/jwks.json` for 1h when no
`jwk` is passed.
...
Max TTL 30 days. Revocation is instant — the verifier rejects any
envelope signed after revocation regardless of the agent's state.
...
`kyc_tier_upgrade`, etc. Class is determined by `SUBTYPE_CLASS`;
cashback is computed via `computeFees()`; the envelope is recorded
under your `project_key` and shows up in `/developer/projects/[id]/events`.
...
`project_key` field in v0.5.0 — when set, sessions and payments are
attributed to the project and recorded the same way.
...
- **npm**: [@orangecheck/me-client](https://www.npmjs.com/package/@orangecheck/me-client)
- **source**: [oc-packages/me-client](https://github.com/orangecheck/oc-packages/tree/main/me-client)
- **canonical type module**: `@orangecheck/me-client/types` (mirror of `oc-me-web/src/lib/events/types.ts`)
