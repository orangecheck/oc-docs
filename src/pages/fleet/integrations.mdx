...
BIP-322-signed action receipt that flows into your fleet project
automatically.
...
| [`@orangecheck/agent-fleet-client`](https://www.npmjs.com/package/@orangecheck/agent-fleet-client) | Shared HTTP client (used by every adapter; install directly if you're rolling your own) | `0.1.0+` |
| [`@orangecheck/webhook-verify`](https://www.npmjs.com/package/@orangecheck/webhook-verify)             | Drop-in HMAC verifier for receiving fleet webhooks                                    | `0.1.0+` |
...
back `null`. The post is _fire-and-forget after_ the call, so a flaky fleet
...
When your fleet project has webhook endpoints subscribed, every accepted
...
See [Webhooks](/fleet/webhooks) for the full event catalog and the delivery /
retry semantics.
...
The client also exposes `postDelegationToFleet`, `postRevocationToFleet`,
and `postSubdelegationToFleet` if you prefer to drive the registry yourself
