...
A **sealed vault** is a single canonical JSON object called an *envelope*. It's self-contained — you can transport it in a URL fragment, a file, a QR code, a Nostr event, or a pigeon. This page walks the format end-to-end. For the normative rules, see the [specification](/lock/spec).
...
The envelope must be byte-identical across clients or the `id` won't match and verification fails. Rules per [SPEC §5](/lock/spec):
...
This is RFC 8785 JCS plus the one `recipients[]` sort rule. `@orangecheck/lock-core` ships the canonicalizer; if you implement one in a new language, cross-check against the [committed test vectors](https://github.com/orangecheck/oc-lock-protocol/tree/main/test-vectors) (`v01-minimal.json` … `v04-with-hint.json`).
