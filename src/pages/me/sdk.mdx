export const metadata = {
    ...
    description:
        '@orangecheck/me-client v0.7.0 SDK reference. Three entry points: main (React surface), /server (framework adapters — Next.js, Express, Hono), /popup (browser-only signin orchestrator). useOcSession, oc.session, oc.payment, oc.event, oc.webhook, oc.config — plus every type the SDK exports.',
};

# `@orangecheck/me-client` · SDK reference

> **Full reference:** [docs.ochk.io/sdk/me-client](/sdk/me-client) — auto-generated from the TypeScript source on every release.
...
[me.ochk.io/me/projects/`<key>`](https://me.ochk.io/me/projects) in the UI);
...
> [agent.ochk.io](https://agent.ochk.io) / [docs.ochk.io/agent](/agent), not
> on me.ochk.io. me-client doesn't ship a delegation namespace; reach for
> `@orangecheck/agent-client` if you need to issue or revoke agent
> delegations from your integration.
...
sites. To read additional fields (their email, attest tier, bitcoin
address, display name) you must explicitly request a scope and the
user must explicitly consent.

Full design at [PRIVACY-ARCHITECTURE.md](https://github.com/orangecheck/oc-me-web/blob/main/PRIVACY-ARCHITECTURE.md).

### `oc.scope.granted`

Read what scopes the currently-signed-in user has already granted to
your project. Returns the per-integrator `sub` plus any granted
field values.
...
browser returns to `return_to`. Subsequent `oc.scope.granted()` calls
...
The user picks **grant once** (1h expiry), **grant always** (until
revoked), or **deny** per scope. Every grant is revocable any time
on `me.ochk.io/me/identity`.
...
2. Switch from `event.user_address` → `event.sub` for user records in
   your database.
...
The `user_address` field is retired from outgoing payloads in the
matching me.ochk.io deploy. Existing rows in your database keyed on
old `user_address` values stay valid; just expect new events to use
`sub`.
