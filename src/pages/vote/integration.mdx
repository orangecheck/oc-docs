export const metadata = {
    title: 'integration',
    description: 'Concrete snippets for gating your community, DAO, forum, or app on an OC Vote poll.',
};

# integration

Concrete snippets for gating your community, DAO, forum, or app on an OC Vote poll. Every example uses [`@orangecheck/vote-core`](https://npmjs.com/package/@orangecheck/vote-core) + [`@orangecheck/vote-cli`](https://npmjs.com/package/@orangecheck/vote-cli) + the curl-friendly `/api/tally` endpoint.

## fork-ready templates

Three copy-forkable integrations live at [**`orangecheck/oc-vote-examples`**](https://github.com/orangecheck/oc-vote-examples):

- [**Discord bot**](https://github.com/orangecheck/oc-vote-examples/tree/main/discord-bot) — `/poll <id>` + `/poll-watch <id>` slash commands that render tallies as embeds with bar charts. Stateless. ~200 lines of discord.js.
- [**GitHub Action gate**](https://github.com/orangecheck/oc-vote-examples/tree/main/github-action-gate) — `uses: orangecheck/oc-vote-examples/github-action-gate@main`. Gates a job on a poll's tally via a configurable jq expression. Outputs `state` + `passed` + `tally-json`.
- [**Shell recipes**](https://github.com/orangecheck/oc-vote-examples/tree/main/shell) — one-liner deploy-gates, create/vote/reveal flows, cron pollers, discovery — pure bash + `npx @orangecheck/vote-cli`.
...
- [`/vote/weight-modes`](/vote/weight-modes) — picking the right weight function.
