export const metadata = {
    ...
    description:
        'Unified docs for the OrangeCheck ecosystem — six open protocols (Attest, Lock, Stamp, Vote, Agent, Pledge), two commercial products (Me, Console), and the company-wide charter. Shared concepts up top, protocol and product sections below.',
};
...
ships two managed commercial products. They use the same envelopes, the same
BIP-322 signing, the same anchor pipeline — they're consumers of the protocol,
...
| **[OC Console](/console)** | Managed enterprise tier for OC Agent and OC Pledge. Integration adapters, webhook signing service, audit-bundle generation, compliance review. Same envelopes as the open protocol path. | enterprises running AI agents under bounded authority |

Both products operate under the same [charter](/charter) — eight commitments
that bind every OrangeCheck surface (no token, custody you can graduate out of,
the protocol stays open, audit verifies without us, and so on).
...
- **[OC Me](/me) · [OC Console](/console)** — the two commercial products. Each
  section follows: overview → quickstart → SDK reference → integrations →
  webhooks → API → custody/operations.
...
- [**OC Console overview**](/console) — managed Agent + Pledge for enterprises
