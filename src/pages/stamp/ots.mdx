...
// Already shaped as stamp-core's verifyOtsAnchor(proofB64, blockHeight, blockHash, envelopeId).
...
bring their own parser, or install `opentimestamps` next to
`@orangecheck/stamp-ots` and call `makeDefaultAnchorVerifier({ headerSource })`,
which loads it lazily. Consumers who only need submission get a dep-light
package.
