export const metadata = {
    title: 'why vote',
    description: 'Every open community in 2026 — Nostr relays, DAOs, grant committees, Bitcoin protocol debates, forum moderation bodies — eventually runs into the same problem: "how do we take a group decision without',
};
...
The complete hypothesis-stress-test lives in [`oc-vote-protocol/WHY.md`](https://github.com/orangecheck/oc-vote-protocol/blob/main/WHY.md). It walks 13 load-bearing claims — from "one address = one ballot is enforceable without a server" to "secret ballots achievable with creator-commit-reveal" to "the Ed25519 substitution test" — and stress-tests each against alternatives. If you're evaluating whether this protocol fits your use case, read that document before the spec.
...
These are explicit non-goals, not things we hope to sneak in later. See [security](/vote/security).
