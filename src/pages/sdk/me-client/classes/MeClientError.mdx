export const metadata = {
    title: "Class: MeClientError",
    description: "Auto-generated API reference for Class: MeClientError. Source: TypeScript types in oc-packages.",
};

[**@orangecheck/me-client**](../README)

***

[@orangecheck/me-client](../README) / MeClientError

# Class: MeClientError

Defined in: [me-client/src/transport.ts:113](https://github.com/orangecheck/oc-packages/blob/main/me-client/src/transport.ts#L113)

## Extends

- `Error`

## Constructors

### Constructor

```ts
new MeClientError(
   message: string, 
   status: number, 
   opts?: {
  errorCode?: string;
  retryAfterSeconds?: number;
}): MeClientError;
```

Defined in: [me-client/src/transport.ts:124](https://github.com/orangecheck/oc-packages/blob/main/me-client/src/transport.ts#L124)

#### Parameters

| Parameter | Type |
| ------ | ------ |
| `message` | `string` |
| `status` | `number` |
| `opts?` | \&#123; `errorCode?`: `string`; `retryAfterSeconds?`: `number`; \&#125; |
| `opts.errorCode?` | `string` |
| `opts.retryAfterSeconds?` | `number` |

#### Returns

`MeClientError`

#### Overrides

```ts
Error.constructor
```

## Properties

| Property | Modifier | Type | Description | Inherited from | Defined in |
| ------ | ------ | ------ | ------ | ------ | ------ |
| &lt;a id="property-errorcode">&lt;/a> `errorCode` | `readonly` | `string` \| `undefined` | Server-side error code (the `error` field on the JSON body, e.g. `project_rate_limit_exceeded`). Stable across versions; safe to match on. The human-readable `message` may change. | - | [me-client/src/transport.ts:118](https://github.com/orangecheck/oc-packages/blob/main/me-client/src/transport.ts#L118) |
| &lt;a id="property-message">&lt;/a> `message` | `public` | `string` | - | `Error.message` | node\_modules/typescript/lib/lib.es5.d.ts:1077 |
| &lt;a id="property-name">&lt;/a> `name` | `public` | `string` | - | `Error.name` | node\_modules/typescript/lib/lib.es5.d.ts:1076 |
| &lt;a id="property-retryafterseconds">&lt;/a> `retryAfterSeconds` | `readonly` | `number` \| `undefined` | Seconds to wait before retrying. Set when the server responded 429 with a Retry-After header (or a `retry_after_seconds` body field). Undefined for any other status. Use with `withRateLimitRetry()` or your own backoff loop. | - | [me-client/src/transport.ts:123](https://github.com/orangecheck/oc-packages/blob/main/me-client/src/transport.ts#L123) |
| &lt;a id="property-stack">&lt;/a> `stack?` | `public` | `string` | - | `Error.stack` | node\_modules/typescript/lib/lib.es5.d.ts:1078 |
| &lt;a id="property-status">&lt;/a> `status` | `readonly` | `number` | - | - | [me-client/src/transport.ts:114](https://github.com/orangecheck/oc-packages/blob/main/me-client/src/transport.ts#L114) |

## Accessors

### isRateLimited

#### Get Signature

```ts
get isRateLimited(): boolean;
```

Defined in: [me-client/src/transport.ts:137](https://github.com/orangecheck/oc-packages/blob/main/me-client/src/transport.ts#L137)

True for the 429 surface · convenience boolean so callers don't
 have to compare status === 429 every time.

##### Returns

`boolean`
