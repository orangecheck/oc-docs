export const metadata = {
    ...
    description:
        'OAuth-peer pattern · sample IntegratorPriceConfig archetypes · zero-bitcoin-knowledge user flow.',
};

# OC Me · Integrations

**OC sits as a peer next to your existing OAuth providers.** A first-time user
picking OC needs no wallet, no seed phrase and no "what is a sat" conversation —
sat-earning starts on first session and accrues to their identity as a signed,
verifiable balance. That balance is held by OrangeCheck during bring-up; see
[custody](/me/federation) for who holds what and what replaces it.
...
export function SignInOptions() {
    ...
    async function onOc() {
        ...
        setBusy(false);
        if (!result) return;
        // Your backend verifies the token and starts your own session.
        const r = await fetch('/api/auth/session', {
            method: 'POST',
            headers: { Authorization: `Bearer ${result.token}` },
        });
        if (r.ok) location.assign('/dashboard');
    }
    return (
...
where it would settle, and [me.ochk.io/custody](https://me.ochk.io/custody) says
who holds it today. None of this is in the OAuth path. None of it surfaces
