export const metadata = {
    title: 'api',
    description: 'The web UI is convenience. The curl output is the authoritative result.',
};
...
```json
{
  ...
  "tallies": { "split_a": 812300000, "split_b": 1102900000, "split_c": 899100000 },
  "ballot_count": 47
}
```
...
import {
  ...
  pollId, ballotId, revealId,
  // secret-mode commit
  commit, buildCommitMessage,
  // weight computation
  voterWeight, ageDays, qualifyingUtxos, totalQualifyingSats, isSupportedMode,
  // the tally
  tally,
  // types
  type Poll, type Ballot, type Reveal, type TallyResult, type Utxo,
...
const draft: Poll = {
  ...
  min_sats: 100_000, min_days: 30,
  mode: 'public', reveal_pk: null,
...
The library is **pure**. Same inputs → byte-identical output across implementations. 28/28 tests pass against the canonical fixtures in [`oc-vote-protocol/test-vectors/`](https://github.com/orangecheck/oc-vote-protocol/tree/main/test-vectors).
