...
5. Client derives `id = sha256(canonical_message)`, submits to two OTS
   calendars.
...
A platform accepts submissions from anyone but wants a stake signal. With
stamps, the stake is **time-pinned**: the platform can require that the signer
held stake _at the moment they wrote the content_, not just right now.
...
`.stamp` envelope carrying _who proposed, when, at what stake_ — and the DAO's
later vote (via [OC Vote](https://vote.ochk.io)) can reference the stamp id.
